...
- (void)layoutSubviews
{
    ...
    self.promptLabel.textColor = kWhiteColor;
...
-(void)scanAnimation
{
    if (_upOrdown == NO) {
        _num ++;
        _line.frame = CGRectMake((ScreenWidth - ScanWidth)/2, ScanY + 2*_num * kFontProportion, ScanWidth, 3);
        if ((2*_num * kFontProportion == ScanWidth )||(2*_num * kFontProportion == ScanWidth-1 * kFontProportion)) {
            _upOrdown = YES;
        }
    }
    else {
        _num --;
        _line.frame = CGRectMake((ScreenWidth - ScanWidth)/2, ScanY+ScanWidth-2*_num * kFontProportion, ScanWidth, 3);
        if (_num == 0) {
            _upOrdown = NO;
        }
    }
//    if (_upOrdown == NO) {
//        _num ++;
//        _line.frame = CGRectMake((ScreenWidth - ScanWidth)/2, ScanY+2*_num * kScreenHeightProportion, ScanWidth, 2 * kScreenHeightProportion);
//        if (2 * _num * kScreenHeightProportion == 244 * kScreenHeightProportion) {
//            _upOrdown = YES;
//        }
//    }
//    else {
